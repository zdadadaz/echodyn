        ...
        with tqdm.tqdm(total=len(dataloader)) as pbar:
            for (i, (X, (large_trace, small_trace, ef, flow), fid)) in enumerate(dataloader):
                ...
                if blocks is None:
                    ...
                    outputs_flow = torch.cat([tmp[j][2] for j in range(tmp.shape[0])])
                ...
                total_flow_avg = total_flow/n / 112 / 112
                
                epoch_loss = total_seg + runningloss_ef/n + total_flow_avg
                
                pbar.set_postfix_str("tot: {:.4f}, ef: {:.4f}, seg: {:4f}, flow: {:4f}".format(epoch_loss, runningloss_ef/n, total_seg, total_flow_avg))
