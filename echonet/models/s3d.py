...
class S3DG(nn.Module):
    ...
    def __init__(self, num_classes=400, dropout_keep_prob = 1, input_channel = 3):
        ...
        self.features = nn.Sequential(
            ...
            nn.MaxPool3d(kernel_size=(2, 2, 2), stride=(1, 1, 1), padding=(0, 0, 0)), # (832, 8, 7, 7)
            Mixed_5b(), # (832, 8, 7, 7)
            Mixed_5c(), # (1024, 8, 7, 7)
            nn.AvgPool3d(kernel_size=(2, 6, 6), stride=1),# (1024, 8, 1, 1)
            nn.Dropout3d(dropout_keep_prob),
            nn.Conv3d(1024, num_classes, kernel_size=1, stride=1, bias=True),# (400, 8, 1, 1)
#             nn.AvgPool3d(kernel_size=(6, 1, 1), stride=1), # (1024, 6, 1, 1)
        )
...
def model_s3d(pretrain_path, num_classes,dropout_keep_prob):
    if pretrain_path:
        model=S3DG(num_classes=400,dropout_keep_prob=dropout_keep_prob)
        model.load_state_dict(pretrain_path)
        model.features[18] = nn.Conv3d(1024, num_classes, kernel_size=(6,1,1), stride=1, bias=True)
        model.features[18].bias.data[0] = 55.6
        return model
    else:
        return S3DG(num_classes=num_classes,dropout_keep_prob=dropout_keep_prob)
...
#         print ('No weights Found! please download first, or comment 382~384th line')
    data=torch.autograd.Variable(torch.rand(2,3,32,112,112)).cuda()
    model = model_s3d(pretrain_path, 1,0.3)
    model=model.cuda()
#     print(model.features[18].bias.data)
