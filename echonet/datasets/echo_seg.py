...
class Echo(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.segmentation is not None:
            ...
            inverseSeg = video[0,...]- video[0,...].mean()
            tmp = inverseSeg
            inverseSeg[tmp!=0] = -inverseSeg[tmp!=0]
#             not sure, need test
            inverseSeg = (inverseSeg - inverseSeg.mean())/inverseSeg.var()
            video[3, :inverseSeg.shape[0], :, :] = inverseSeg
