...
class Echo2d_bt(torch.utils.data.Dataset):
    def __init__(self, root=None,
        ...
        if split == "external_test":
            ...
            self.low = collections.defaultdict(list)
            self.high = collections.defaultdict(list)
...
#                         print(" hs filename",len(self.low[filename]))
                        if len(self.high[filename])==0:
                            ...
                            self.high[filename][0] = f
                            self.high[filename][1] = s
