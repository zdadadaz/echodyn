...
# +
class Echo_tsn(torch.utils.data.Dataset):
    def __init__(self, root=None,
        ...
        if split == "external_test":
            ...
            self.peak = self.frames = collections.defaultdict(list)
            with open(self.folder / "seg_size.csv") as f:
                header = f.readline().strip().split(",")
                for (i, line) in enumerate(f):
                    filename, f, s, hl, hs, cs = line.strip().split(',')
                    f  = int(f)
                    s  = int(s)
                    hl = int(hl)
                    hs = int(hs)
...
#         print(video.shape)
        if self.pad is not None:
            if self.crops == 1:
                ...
                p, c, l, h, w = video.shape
                temp = np.zeros((p, c, l, h + 2 * self.pad, w + 2 * self.pad), dtype=video.dtype)
                temp[:, :, :, self.pad:-self.pad, self.pad:-self.pad] = video
                i, j = np.random.randint(0, 2 * self.pad, 2)
                video = temp[:, :, :, i:(i + h), j:(j + w)]
        return video, target
